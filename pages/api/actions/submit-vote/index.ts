...
import { GAME_TYPE, getGameID, IBattleById, ICreateBattle } from "./types";
...
const getHandler = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    ...
    const usernamesQuery = req.query.usernames as string;
    const usernames = usernamesQuery?.split(",").map((username) => username.trim());

    if (!usernames || usernames.length === 0) {
      return res.status(400).json({ error: "No usernames provided" });
    }
    logger.info("Usernames parsed: %o", usernames);
    ...
    const challenge: IBattleById = challengeResponse.data.data;
    ...
    const options = usernames.map((username, index) => ({
      label: username,
      value: `user_${index}`,
      selected: index === 0,
    }));
